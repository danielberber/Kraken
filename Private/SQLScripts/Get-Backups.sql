...
row_number() over(partition by database_name, type, device_type, is_copy_only order by backup_start_date desc) rn,
compressed_backup_size, user_name
FROM msdb.dbo.backupset b JOIN msdb.dbo.backupmediafamily m 
ON b.media_set_id = m.media_set_id) q right join DBs d
