    ...
    Process {
        foreach ($SQLInstance in $ServerInstance) {
            ...
            if ($PSBoundParameters.Keys -contains "SQLConnection") {
                ...
                if ($Credential) {
                    $ConnectionString = "Server={0};Database={1};User ID={2};Password=`"{3}`";Trusted_Connection=False;Connect Timeout={4};Encrypt={5};MultiSubnetFailover=True" -f $SQLInstance, $Database, $Credential.UserName, $Credential.GetNetworkCredential().Password, $ConnectionTimeout, $Encrypt
                }
                else {
                    $ConnectionString = "Server={0};Database={1};Integrated Security=True;Connect Timeout={2};Encrypt={3};MultiSubnetFailover=True" -f $SQLInstance, $Database, $ConnectionTimeout, $Encrypt
                }
